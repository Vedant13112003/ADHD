...
class FMRIDataGenerator(keras.utils.Sequence):
    ...
    def pad_image(self, img):
        original_img = img.get_fdata()
        print(f"Original image shape: {original_img.shape}")

        # Resize if necessary
        if original_img.shape[:3] != (self.x_dim, self.y_dim, self.z_dim):
            print(f"Resizing image from {original_img.shape[:3]} to {(self.x_dim, self.y_dim, self.z_dim)}")
            resized_img = zoom(original_img[:, :, :, :], 
                            (self.x_dim / original_img.shape[0], 
                                self.y_dim / original_img.shape[1], 
                                self.z_dim / original_img.shape[2], 
                                1),  # Keep time dimension unchanged
                            order=1)
            print(f"Resized image shape: {resized_img.shape}")
        else:
            resized_img = original_img

    # Padding logic remains unchanged...


        # Create padding
        img_padding = np.expand_dims(np.zeros((self.x_dim, self.y_dim, self.z_dim)), axis=3)
        amt_to_fill = self.time_length - resized_img.shape[3]
